-- schema.sql — 데이터베이스 테이블 생성 SQL
-- Week 09 — MyBatis + DB 연동
-- MySQL 8.x 기준

-- 데이터베이스 생성 (Docker 환경에서는 자동 생성됨)
...
-- ============================================
-- 사용자 테이블 (로그인/회원 관리용)
-- Week 07 — 세션 기반 로그인에서 활용
-- ============================================
CREATE TABLE IF NOT EXISTS users (
    id         BIGINT       NOT NULL AUTO_INCREMENT   COMMENT '사용자 PK',
    username   VARCHAR(50)  NOT NULL UNIQUE           COMMENT '로그인 아이디',
    password   VARCHAR(255) NOT NULL                  COMMENT '비밀번호 (실무에서는 BCrypt 해시)',
    name       VARCHAR(100) NOT NULL                  COMMENT '사용자 이름 (화면 표시용)',
...
-- ============================================
-- 게시판 테이블 (게시글 CRUD 대상)
-- Week 10 — Spring MVC 패턴 게시판
-- Week 11 — 첨부파일 컬럼 추가 (file_name, saved_name)
-- ============================================
CREATE TABLE IF NOT EXISTS board (
    id         BIGINT       NOT NULL AUTO_INCREMENT                               COMMENT '게시글 번호 PK',
    title      VARCHAR(200) NOT NULL                                              COMMENT '제목',
    content    TEXT         NOT NULL                                              COMMENT '내용',
    author     VARCHAR(100) NOT NULL                                              COMMENT '작성자',
    created_at DATETIME     DEFAULT CURRENT_TIMESTAMP                             COMMENT '작성일',
    updated_at DATETIME     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '수정일',
    file_name  VARCHAR(255) NULL                                                  COMMENT '원본 파일명 (첨부파일 없으면 NULL)',
    saved_name VARCHAR(255) NULL                                                  COMMENT '서버 저장 파일명 (UUID_원본명 형식)',
    PRIMARY KEY (id),
    INDEX idx_created_at (created_at DESC)          -- 최신순 조회 성능 개선
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='게시판 테이블';

-- ============================================
-- 기존 board 테이블에 파일 컬럼을 추가하는 경우 (ALTER)
-- Week 11 — 이미 board 테이블이 존재할 때 실행
-- ============================================
-- ALTER TABLE board
--     ADD COLUMN file_name  VARCHAR(255) NULL COMMENT '원본 파일명' AFTER updated_at,
--     ADD COLUMN saved_name VARCHAR(255) NULL COMMENT '서버 저장 파일명 (UUID)' AFTER file_name;
